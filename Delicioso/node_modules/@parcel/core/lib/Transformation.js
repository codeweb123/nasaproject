...
class Transformation {
  ...
  async runPipeline(pipeline, initialAsset) {
    ...
    if (generate != null) {
      await Promise.all(resultingAssets.filter(asset => asset.ast != null && !(asset.value.env.scopeHoist && asset.value.type === 'js' || this.options.mode === 'production' && asset.value.type === 'css' && asset.value.symbols)).map(async asset => {
        if (asset.isASTDirty) {
          var _output$map;
